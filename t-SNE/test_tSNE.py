            ...
            for ccle in range(11,len(self.header)-1):
                if i < len(self.raw_PCA_data.keys()):
                    for event in self.reduced_PCA_data:
                        try:
                            self.PCA_data_tissue[self.header[ccle]].append(self.reduced_PCA_data[event][i])
                        except KeyError:
                            self.PCA_data_tissue[self.header[ccle]] = [self.reduced_PCA_data[event][i]]
                    i += 1
    ...
    def do_tSNE(self):
        ...
        for tissue in self.tissue_index:
            for index in self.tissue_index[tissue]:
                plt.scatter(X[index-11][0], X[index-11][1], color = colors[tissue], label = self.header[index], marker = '.', s = 60, alpha = 0.5)
